  ...
  sourceFiles.forEach((file) => {
    ...
    classesInFile.forEach((cls) => {
      ...
      methods.forEach((method) => {
        compareWithClassesSameOrOtherFiles(method, cls, file.getFilePath());
...
function compareWithClassesSameOrOtherFiles(method, clazz, filepath) {
...
function analyzeMethodReferences(
  ...
  references.forEach((ref) => {
    ...
    if (!refClass) {
      // The reference might not be inside a class (it could be in a function or a variable, for example)
      return;
    }
